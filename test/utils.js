...
module.exports = ({ web3 } = {}) => {
  ...
  const toPreciseUnit = (amount) => {
    ...
    if (amount === '.') {
      throw new Error(
        `Error converting number ${amount} to precise unit, invalid value`
      );
    }
    ...
    if (rest.length > 0) {
      throw new Error(
        `Error converting number ${amount} to precise unit, too many decimal points`
      );
    }
    ...
    if (fraction.length > PRECISE_UNIT_STRING.length - 1) {
      throw new Error(
        `Error converting number ${amount} to precise unit, too many decimal places`
      );
    }
